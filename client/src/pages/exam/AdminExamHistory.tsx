...
interface AdminExamHistoryItem {
  userId: string;
  userName?: string;
...
export default function AdminExamHistory() {
  const [items, setItems] = useState<AdminExamHistoryItem[]>([]);
  ...
  const load = async (page = 1, search = "") => {
    ...
    try {
      const res = await api.get(
        `/user-exams/history/all?page=${page}&limit=${data.limit}&search=${encodeURIComponent(
          search
        )}`
      );
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <h1 className="text-2xl font-semibold">All Exam History</h1>
      ...
      <div className="flex gap-2 items-center">
        <input
          ...
          placeholder="Search user, course, exam..."
